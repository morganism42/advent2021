rawtext = ' '
example = '8A004A801A8002F478'
binary = bin(int(example, 16))[2:]
while len(binary) % 4 != 0:
    binary = '0' + binary
length = []
versions = []


def literal(binary, position, num, length):  # ID 4
    length[-1] += 5
    if binary[position] == '1':
        num += binary[position + 1:position + 5] + literal(binary, position + 5, num, length)
    else:
        num += binary[position + 1:position + 5]
    return num


def operator(binary, num, length):
    length[-1] += 1
    if binary[length[-1] - 1] == '1':
        packets = int(binary[7:18], 2)
        pack = binary[18:]
        for i in range(packets):
            pack, num, lengthtemp = unpack(pack, length)
            length[-1] += lengthtemp
    else:
        packetlen = int(binary[7:22], 2)
        length[-1] += packetlen+15
        pack = binary[22:packetlen + 22]
        while len(pack) > 0:
            pack, num, lengthtemp = unpack(pack, length)
    return num


def unpack(binary, length):
    length.append(6)
    print(binary)
    versions.append(int(binary[0:3], 2))
    ID = int(binary[3:6], 2)
    if ID == 4:
        num = literal(binary, 6, '', length)
        num = int(num, 2)
    else:
        num = operator(binary, '', length)
        num = int(num, 2)
    binary = binary[length[-1]:]
    lengthtemp = length[-1]
    del length[-1]
    return binary, bin(num)[2:], lengthtemp


while '1' in binary and len(binary) > 3:
    print('hi')
    binary, num, lengthtemp = unpack(binary, length)
print(sum(versions))
